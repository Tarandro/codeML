...
import requests
import pandas as pd
from exploration_streamlit import Eda_NLP
from eda_utils import Flags_EDA

### run application : streamlit run dashboard.py

### with docker compose : go to http://host.docker.internal:8501
...
# url api from app.py :
url = "http://host.docker.internal:5000/predict" #"http://host.docker.internal:5000/predict" #'http://127.0.0.1:5000/predict'
...
if Section == 'Prediction':
    ...
    if btn:
        ...
        if data is None:
            ...
            resp = requests.get(url, files=params, verify=False)
