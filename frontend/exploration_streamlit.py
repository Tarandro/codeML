...
class Eda_NLP:
    ...
    def data_preprocessing(self, dataset=None, copy=True):
        ...
        if self.column_date in self.data:
            if isinstance(self.data[self.column_date].iloc[0], datetime.date):
                pass
            else:
                self.data[self.column_date] = self.data[self.column_date].apply(lambda x: datetime.datetime.strptime(x, '%Y-%m-%d'))
    ...
    def show_number_docs(self):
        ...
        try:
            ...
            return "Number of non empty texts in columns '{}' : {}".format(self.column_text, number_texts)
        except:
            return None
    ...
    def show_number_unique_words(self, column_text="clean_text"):
        ...
        try:
            ...
            return "Number of unique words in columns '{}' : {}".format(column_text, number_unique_words)
        except:
            return None
    ...
    def show_range_date(self):
        ...
        try:
            return 'Documents dating between {} and {}'.format(self.data[self.column_date].min().date(), self.data[self.column_date].max().date())
        except:
            return None
    ...
    def show_top_n_words(self, column_text="clean_rsw_text", n=10):
        ...
        try:
            ...
            return "Top {} words : {}".format(n, list(df_ngrams.terms))
        except:
            return None
    ...
    def show_summary(self):
        ...
        text = " "
        text1 = self.show_number_docs()
        if text1 is not None: text += text1 + "\n"
        text2 = self.show_number_unique_words()
        if text2 is not None: text += text2 + "\n"
        text3 = self.show_top_n_words()
        if text3 is not None: text += text3 + "\n"
        text4 = self.show_range_date()
        if text4 is not None: text += text4
        return text
            ...
            for term in terms:
                ...
                for doc in self.data[column_text]:
                    ...
                    if count != 0:
                        n_count += count
                        n_documents += 1
                text += "Term '{}' appears in {} documents with a total count of {}\n".format(term, n_documents, n_count)
            return text
        except:
            return None
