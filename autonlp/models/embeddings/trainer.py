...
class Embedding:
    ...
    def fit_transform_vectorization_tf_tfidf(self, x_train_preprocessed):
        """ Fit and Transform for TF/TF-IDF method and dimension reduction with SparseRandomProjection if
            tf_wde_vector_size/tfidf_wde_vector_size < size features TF/TF-IDF
        Args:
            x_train_preprocessed (array)
        Return:
            doc_vectors (array) document vectors
        """
    ...
    def transform_vectorization_tf_tfidf(self, x_preprocessed):
        """ Transform for TF/TF-IDF method and dimension reduction with latest fit
        Args:
    ...
    def fit_transform(self, x_train_before, x_val_before=None, method_embedding=None,
        ...
        Return:
            dict_preprocessed (dict) with x_train_before and x_val_before preprocessed by embedding method
        """

        outdir_model = os.path.join(self.flags_parameters.outdir, 'embedding', self.embedding.name_model)
        file_to_read = os.path.join(outdir_model, "{}.pickle".format("dict_preprocessed_train"))
        ...
        if os.path.exists(file_to_read):
            try:
                ...
                dict_preprocessed = pickle.load(file_to_read)
        ...
        outdir_model = os.path.join(self.flags_parameters.outdir, 'embedding', self.embedding.name_model)
        file_to_write = open(os.path.join(outdir_model, "{}.pickle".format("dict_preprocessed_train")), "wb")
        pickle.dump(dict_preprocessed, file_to_write)
    ...
    def transform(self, x_test_before_copy, doc_spacy_data_test=[]):
        ...
        Return:
            dict_preprocessed (dict) with x_test_before_copy preprocessed by embedding method
        """
        x_test = x_test_before_copy.copy()

        try:
            _ = self.embedding.method_embedding
        except:
            outdir_file_embedding = os.path.join(self.flags_parameters.outdir, 'embedding')
            outdir_embedding = os.path.join(outdir_file_embedding, self.embedding.name_model)
            with open(os.path.join(outdir_embedding, "parameters_embedding.json")) as json_file:
