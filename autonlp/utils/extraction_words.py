...
def extract_influent_word(X, Y, model_nlp, loaded_models, model_paths, X_is_train_data=False, n_influent_word=10, nb_example='max', pr=None):
    """ extraction of influential words : highlight the words with the highest weight
        for word encoder model with attention classification layer and 'transformer+global_average' model
    Args:
    ...
    # if a Attention layer is use in the classification layers so use the weights of this layer
    if "attention" in model_nlp.name_classifier.lower():
        name_model_att = True
    # else for Transformers : use Attention weights from last Attention layer of Transformer model
    else:
    ...
    # Tokenization Transformer or Tensorflow, and save token for punctuation
    ...
    # with have different fold model, so we recreate cross validation to get fold models :
    ...
    for num_fold, (train_index, val_index) in enumerate(folds):
        ...
        # Instantiate model that return attention weights :
        if name_model_att:
        ...
        for k in tqdm(val_index):
            ...
            # calcul to do to get weight for each word :
            if name_model_att:
                # USE EXTRACT MODEL
                if model_nlp.embedding.name_model == "transformer":
...
def get_top_influent_word(dict_result, model_nlp, n_gram=2, top_k=20, min_threshold=10):
    """ use the output of the previous function to get terms with highest average weights for each class """
    ...
    # special token of Transformer or Tensorflow Tokenization
    if model_nlp.embedding.name_model == "transformer":
