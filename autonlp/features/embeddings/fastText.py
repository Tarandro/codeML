    ...
    def preprocessing_fit_transform(self, x, size_params='small', method_embedding='fasttext'):
        ...
        if self.method_embedding.lower() == 'fasttext':
            ...
            if not os.path.exists(dir_fasttext):
                ...
                build_fasttext_model(x_preprocessed, output_dir=dir_fasttext,
        ...
        try:
            try:
                self.embeddings_index = load_embeddings(self.method_embedding)
            except:
                self.embeddings_index = load_keyedvectors(self.method_embedding)
        except Exception:
        ...
        if self.dimension_embedding == 'word_embedding':
            ...
            self.embedding_matrix = build_embedding_matrix_from_embeddings_index(self.word_index, self.embeddings_index)
            self.embed_size = self.embedding_matrix.shape[1]

            return x_preprocessed

        else:
            document_embedding = build_embedding_documents_from_embeddings_index(x_preprocessed,
...
def build_embedding_documents_from_embeddings_index(documents, embeddings_index, language_text="fr", lower=True, verbose=True):
    ...
    for i, doc in tqdm(enumerate(documents), disable=not verbose):
        ...
        try:
            ...
            for word in doc_split:
                vect_word = get_vect(word, embeddings_index)
