...
class Tf_embedding(Base_Embedding):
    """ Base_Embedding class with TF embedding method from sklearn """
    ...
    def hyper_params(self, size_params='small'):
        ...
        if self.dimension_embedding == 'doc_embedding':
            # TF embedding
            if self.flags_parameters.tf_binary:
                # self.parameters['vect__tf__binary'] = [True, False]
                self.parameters['vect__tf__binary'] = hp.choice('vect__tf__binary', [True, False])
            else:
                # self.parameters['vect__tf__binary'] = [False]
                self.parameters['vect__tf__binary'] = hp.choice('vect__tf__binary', [False])
            # self.parameters['vect__tf__ngram_range'] = self.flags_parameters.tf_ngram_range
            self.parameters['vect__tf__ngram_range'] = hp.choice('vect__tf__ngram_range', self.flags_parameters.tf_ngram_range)
            if self.flags_parameters.tf_stop_words:
                self.parameters['vect__tf__stop_words'] = hp.choice('vect__tf__stop_words', [None, True])
            else:
                # self.parameters['vect__tf__stop_words'] = [None]
                self.parameters['vect__tf__stop_words'] = hp.choice('vect__tf__stop_words', [None])

        else:
            self.parameters['learning_rate'] = hp.choice('learning_rate', self.flags_parameters.tf_wde_learning_rate)
    ...
    def preprocessing_fit_transform(self, x, doc_spacy_data, method_embedding):
        ...
        if self.dimension_embedding == 'doc_embedding':
            ...
            if self.max_features is None:
                self.max_features = self.flags_parameters.tf_wde_max_features
            if self.maxlen is None:
                self.maxlen = self.flags_parameters.tf_wde_maxlen
            ...
            self.embedding_matrix = build_embedding_matrix_from_tf(self.word_index, x_preprocessed,
                                                                   self.flags_parameters.tfidf_wde_stop_words,
                                                                   self.flags_parameters.language_text,
                                                                   self.flags_parameters.tf_wde_binary,
    ...
    def model(self):
        ...
        if self.dimension_embedding == 'doc_embedding':
            tf_ngrams = CountVectorizer()

            vect = ColumnTransformer(transformers=[
                ('tf', tf_ngrams, self.column_text)
            ])
...
#################
# Help function : Get TF pre-training-weight to get word embeddings
#################


def get_embedding_index_tf(corpus, stopwords=True, language_text='fr', binary=True, vector_size=200, seed=15):
    """ Create a word vector for each word of the TF matrix of the corpus
    ...
    if stopwords:
        ...
        stopwords = None
    vectorizer = CountVectorizer(stop_words=stopwords, ngram_range=(1, 1), binary=binary)
...
def build_embedding_matrix_from_tf(word_index, corpus, stopwords, language_text, binary, vector_size, seed):
    """ Create a word vector for each word in dictionary word_index with TF embedding method
    Args:
    ...
    embedding_index = get_embedding_index_tf(corpus, stopwords, language_text, binary, vector_size, seed)
