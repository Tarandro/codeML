...
from autonlp.models.classifier.global_average import Global_Average


class TestclassGlobalAverage(object):
    """
    ...
    x = pd.DataFrame({flags.column_text: [
        ...
        range(10)]})
    y = pd.DataFrame({flags.target: [0 if i % 2 else 1 for i in range(10)]})
    x_val = pd.DataFrame({flags.column_text: [
        "text" + str(rd.randint(0, 5)) + " text" + str(rd.randint(0, 5)) + " text" + str(rd.randint(0, 5)) for i in
        range(5)]})
    y_val = pd.DataFrame({flags.target: [0 if i % 2 else 1 for i in range(5)]})
    ...
    def test_optimization(self):
        ...
        model = Global_Average(self.flags, Tfidf, 'Tf_idf+Global_Average', self.column_text)
