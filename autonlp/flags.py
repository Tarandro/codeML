...
class Flags:
    ...
    # if you want to use a model from model_deployment directory
    ...
    classifier: dict = field(
        default_factory=lambda: {"Naive_Bayes": [1], "Logistic_Regression": [2], "SGD_Classifier": [],
                                 "XGBoost": [], "Global_Average": [], "Attention": [], "BiRNN": [],
                                 "BiRNN_Attention": [],
                                 "biLSTM": [], "BiLSTM_Attention": [], "biGRU": [], "BiGRU_Attention": []})

    regressor: dict = field(
        default_factory=lambda: {"SGD_Regressor": [1],
                                 "XGBoost": [], "Global_Average": [2], "Attention": [], "BiRNN": [],
                                 "BiRNN_Attention": [],
                                 "biLSTM": [], "BiLSTM_Attention": [], "biGRU": [], "BiGRU_Attention": []})

    clustering: dict = field(
        default_factory=lambda: {"NMF_frobenius": [1], "NMF_kullback": [], "LDA": [],
                                 "hdbscan": [], "ACP_hdbscan": [], "UMAP_hdbscan": [],
                                 "kmeans": [], "ACP_kmeans": [2], "UMAP_kmeans": [],
                                 "agglomerativeclustering": [], "ACP_agglomerativeclustering": [],
                                 "UMAP_agglomerativeclustering": [], "Zero_shot": []})

    # maximum Hyperparameters Optimization time in seconds for each model:
    max_run_time_per_model: int = 60  # -1 to skip this params
    # maximum Hyperparameters Optimization trial:
    max_trial_per_model: int = 1000   # -1 to skip this params
    ...
    #     Transformer Zero shot Classif : 'vicgalle/xlm-roberta-large-xnli-anli' or 'joeddav/xlm-roberta-large-xnli'
    ...
    apply_ray: bool = False
    # number of model to train in parallel
    ray_max_model_parallel: int = 1
    # number of cpu per model
    ray_cpu_per_model: int = 1
    # number of gpu per model
    ray_gpu_per_model: int = 0
    # verbose ray
    ray_verbose: int = 2
    ...
    ### Embedding
    dimension_embedding: str = "doc_embedding"
    ...
    aglc_linkage: str = "ward"  # 'ward', 'complete', 'average', 'single'

    ## unsupervised
    vocabulary_labels: dict = field(
        default_factory=lambda: {"positif": ["positif", "bien"],
                                 "négatif": ["négatif", "mauvais"]})

    ## Zero-shot Classification
    pattern_zero_shot = "Le sentiment du commentaire est {}"  # '{}' will be replaced by vocabulary_labels (average)

    ### Topic Modeling
    show_top_terms_topics: bool = False
    # format (keep_pos_tag, lemmatize). keep_pos_tag can be 'all' for no pos_tag else list of tags to keeps.
    # lemmatize is boolean to know if you wan to apply lemmatization by Spacy model.
    preprocess_topic: tuple = (['ADJ', 'NOUN', 'VERB'], True)
    n_top_words: int = 10
    min_ngram: int = 1
    max_ngram: int = 1
    ...
    tr_maxlen: int = 258
    tr_learning_rate: list = field(default_factory=lambda: [2e-5])
